...
def run_evaluation(env, agents, max_weeks=52):
    ...
    while not done and week < max_weeks:
        ...
        for i, city in enumerate(env.cities):
            city_name = city.name
            mask_val = env.actions[action_indices[i]][0]
            home_val = env.actions[action_indices[i]][1]
            med_val  = env.actions[action_indices[i]][2]
            infection = city.I
...
def run_no_intervention(env, max_weeks=52):
    ...
    no_int_index = None
    for idx, (m, h, d) in enumerate(env.actions):
    ...
    while not done and week < max_weeks:
        ...
        for i, city in enumerate(env.cities):
            city_name = city.name
            mask_val = env.actions[no_int_index][0]
            home_val = env.actions[no_int_index][1]
            med_val  = env.actions[no_int_index][2]
            infection = city.I
            economic_loss = env.economy_history[city.name][-1]
...
def plot_test_results(df_all):
    ...
    for scenario in scenarios:
        ...
        for city in cities:
            ...
            plt.plot(city_data['week'], city_data['infection'],
    ...
    for scenario in scenarios:
        ...
        for city in cities:
            city_data = df_s[df_s['city'] == city]
            plt.plot(city_data['week'], city_data['economic'],
                     label=f"{city}-{scenario}")
